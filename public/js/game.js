// 繧ｰ繝ｭ繝ｼ繝舌Ν縺ｪ繧ｭ繝ｼ繝懊繝峨う繝吶Φ繝
window.addEventListener('keydown', (e) => {
  // オフライン成果ダイアログの対応
  const offlineOverlay = document.getElementById('offline-result-overlay');
  if (offlineOverlay && (offlineOverlay.style.display === 'flex' || offlineOverlay.style.display === 'block')) {
    if (e.key === 'Enter' || e.key === 'Escape') {
      closeOfflineDialog();
    }
  }

  // アドベンチャー画面の対応 (Enter/Spaceで次へ)
  const advOverlay = document.getElementById('adventure-overlay');
  if (advOverlay && (advOverlay.style.display === 'flex' || advOverlay.style.display === 'block')) {
    if (e.key === 'Enter' || e.key === ' ') {
      nextAdventureMsg();
      e.preventDefault(); // スペースでスクロールするのを防止
    }
  }
});
...
window.switchDungeonTab = function (tabName) {
  ...
  // タブの見た目更新
  document.querySelectorAll('.dungeon-tabs .tab-btn').forEach(btn => btn.classList.remove('active'));
...
window.switchStoryTab = function (tabName) {
  // タブの見た目更新
  document.querySelectorAll('.story-tabs .tab-btn').forEach(btn => btn.classList.remove('active'));
...
function renderEquipmentScreen() {
  const parts = {
    hand_r: "右手", hand_l: "左手",
    head_top: "頭上段", head_mid: "頭中段", head_low: "頭下段",
    neck: "首", ear: "耳", body: "体", arm: "腕",
    waist: "腰", leg: "足", foot: "靴", sub1: "その他1", sub2: "その他2"
  };
  ...
  for (let key in parts) {
    ...
    if (rarity > 0) {
      div.style.setProperty("background", `linear-gradient(90deg, ${bgColor} 0%, #ffffff 20%)`, "important");
    } else {
  ...
  // 2. 所持品アイテム
  const filtered = player.inventory.map((item, idx) => ({ item, idx }))
    .filter(entry => {
  ...
  filtered.forEach((entry, i) => {
    ...
    if (!master) {
      ...
      div.style.setProperty("color", textColor, "important");
      if (displayRarity > 0) div.style.setProperty("border-color", "rgba(0,0,0,0.1)", "important");
...
function getRarityBgColor(r) {
  ...
  const bgs = ["#ffffff", "#e2fcd4", "#d4f1ff", "#f3e2ff", "#fff5d4", "#ffd4d4", "#d4ffff", "#f5e6d3"];
  return bgs[r] || "#ffffff";
}
...
function updateStatComparison() {
  const diffs = { hp: 0, str: 0, vit: 0, agi: 0, int: 0, dex: 0, luk: 0, atk: 0, matk: 0, def: 0, mdef: 0, hit: 0, eva: 0, cri: 0, res: 0 };

  // 1. 現在の装備による合計ボーナスを計算
  let currentBonus = { hp: 0, str: 0, vit: 0, agi: 0, int: 0, dex: 0, luk: 0, atk: 0, matk: 0, def: 0, mdef: 0, hit: 0, eva: 0, cri: 0, res: 0 };
      ...
      if (eqItem) {
        ...
        if (eqItem.options) {
          eqItem.options.forEach(o => {
            if (currentBonus[o.stat] !== undefined) currentBonus[o.stat] += Number(o.val);
          });
        }
      }
    }
  ...
  let nextMaster = null;
  const targetIdx = currentHoveredCandidate !== null ? currentHoveredCandidate : currentSelectedCandidate;
  if (targetIdx !== null) {
...
function applySelectedEquip(invIdx) {
  ...
  console.log(`[Equip] applySelectedEquip: Part=${currentSelectedPart}, invIdx=${invIdx}, isUnequipOnly=${isUnequipOnly}`);
...
function getSkillBonuses() {
  let skillBonus = {
    hp: 0, str: 0, vit: 0, agi: 0, int: 0, dex: 0, luk: 0,
    atk: 0, matk: 0, def: 0, mdef: 0, hit: 0, eva: 0, cri: 0, res: 0
  };
...
function calcBattleStats() {
  ...
  // --- 装備補正の集計 ---
  let bonus = {
    hp: skillBonus.hp, str: skillBonus.str, vit: skillBonus.vit, agi: skillBonus.agi,
    int: skillBonus.int, dex: skillBonus.dex, luk: skillBonus.luk,
    atk: skillBonus.atk, matk: skillBonus.matk, def: skillBonus.def,
    mdef: skillBonus.mdef, hit: skillBonus.hit, eva: skillBonus.eva,
    cri: skillBonus.cri, res: skillBonus.res
  };
...
async function saveGame() {
  ...
  // ★変更: サーバーに保存
  if (currentUserEmail) {
    const statusEl = document.getElementById("login-error"); // 簡易的にログインエラー欄を使うか、ヘッダーに表示するか
    // ゲーム画面内に表示エリアがないため、consoleのみにするか、一時的にオーバーレイ出すか。
    // ここでは安全にconsoleのみにしつつ、可能ならCanvasに描画したいが、draw関数外なので難しい。
    // 右上の debug-speed-btn の横にでも出す？
    // ひとまずコンソールログは維持。
    ...
    try {
      ...
      await fetch(API_BASE + "/save_game.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ save_data: JSON.stringify(player) }),
      });
      console.log("Game saved to server.");
      // localStorageにもバックアップとして保存（同期ミス防止のため常に最新をローカルにも）
      localStorage.setItem("cc_save_data_" + currentUserEmail, JSON.stringify(player));
    } catch (e) {
...
async function loadGame() {
  ...
  try {
    ...
    if (data.status === "success" && data.save_data) {
      const loadedPlayer = JSON.parse(data.save_data);
      // ★修正: 保存されていた画像のパス(baby.png)で上書きされるのを防ぐため、
      // コード上の最新のデフォルト画像(player1.png)を優先する。
      // ※将来的に着せ替え機能を実装する場合はロジック要変更
      // ★修正: リバランス等でコード側の初期値を変更しても、セーブデータで上書きされてしまう問題への対応
      // 座標やサイズ、画像パスなどの「ステータス以外の定義値」はコード側の値を優先する
      const defaultProps = {
        image: player.image,
        x: player.x,
        y: player.y,
      ...
      if (localData) {
        player = { ...player, ...JSON.parse(localData) };
        console.log("Game loaded from local storage (Initial sync).");
        // 初回のみ即座にサーバーへ保存して同期を図る
        saveGame();
      }
    }
  } catch (e) {
  ...
  // ロード時にレベルアップ判定を行う (万が一ズレていた場合やオフラインバグの補正)
  checkLevelUp();
}
...
window.addEventListener("DOMContentLoaded", () => {
  const isDev = location.hostname === "localhost" || location.hostname === "127.0.0.1" || location.pathname.includes("/dev/");
  ...
  if (fade.state === "out") {
    ...
    if (fade.alpha <= 0) {
      ...
      if (fade.showFieldTitle) {
        ...
        if (dData) {
          spawnDamageText(0, canvas.height / 2, `- ${dData.name} -`, "#ffffff", 32, false, true);
        }
...
function update() {
  ...
  if (player.attackTimer > currentInterval) {
    ...
    if (target) {
      ...
      if (masterData && masterData.skills) {
        for (let skillId in player.skills) {
          const master = masterData.skills.find(s => s.id == skillId);
          if (master && (!player.skill_cooldowns[skillId] || player.skill_cooldowns[skillId] <= 0)) {
            activeSkill = master;
            break;
          }
        }
      ...
      for (let h = 0; h < hitCount; h++) {
        ...
        // 複数ヒットの場合は少しずつずらして表示
        spawnDamageText(target.x + (h * 5), ey - 20 - (h * 15), dmg, isSkill ? "#f1c40f" : "#ffffff", isSkill ? 24 : 20, isSkill);
      }
      ...
      if (target.hp <= 0) {
        ...
        if (index > -1) {
          ...
          if (target.money > 0) {
            ...
            let ey = canvas.height * target.yRatio;
            spawnDamageText(target.x, ey - 40, `+${target.money} Mumel`, "#f1c40f", 24, false);
            updateUI();
          }
  ...
  if (player.hp <= 0) {
    ...
    startTransition(() => {
      ...
      spawnDamageText(canvas.width / 2, canvas.height / 2, "RETRY...", "#e74c3c", 40, false, true);
    });
  }
}
    ...
    if (id > 0 && weight > 0) {
      ...
      if (pick < weight) {
        ...
        if (itemMaster) {
          ...
          if (isRare) {
            // レアドロップ演出
            spawnDamageText(enemy.x, ey - 60, `🌟RARE GET!!🌟\n${itemMaster.name}`, "#ff00ff", 28, true);
            addCombatLog(`🌟GET: ${itemMaster.name}`, "#ff00ff");
          } else {
            // 通常ドロップ演出
            // 複数ドロップ時に重ならないように少しずらすなどの工夫があっても良いが、一旦位置は固定
            spawnDamageText(enemy.x, ey - 50, `GET: ${itemMaster.name}`, "#f1c40f", 20, false);
...
function generateOptions(rarity) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (selected) {
      ...
      if (!optName) {
        const statMap = { str: "力", vit: "体力", agi: "素早さ", int: "知力", dex: "器用さ", luk: "運" };
...
function draw() {
  ...
  if (pImg && pImg.complete && (pImg.naturalHeight !== 0 || pImg.naturalWidth !== 0)) {
    ctx.drawImage(pImg, player.x, canvas.height * 0.5 - player.height / 2, player.width, player.height);
  } else {
    // console.warn("Player image not ready or invalid:", player.image, pImg);
    ctx.fillRect(player.x, canvas.height * 0.5 - player.height / 2, player.width, player.height);
  }
...
window.renderCombatSkills = function () {
  ...
  let html = "";
  for (let i = 0; i < 3; i++) {
    const skillId = (player.equippedSkills && player.equippedSkills[i]) ? player.equippedSkills[i] : null;

    let content = "";
    let overlayHeight = "0%";

    if (skillId) {
      const skill = masterData && masterData.skills ? masterData.skills.find(s => s.id == skillId) : null;
      const iconUrl = `images/${(skill && skill.image) ? skill.image : 'skill/default.png'}`;
